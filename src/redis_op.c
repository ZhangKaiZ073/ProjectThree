...
int Rop_GetValue(redisContext *conn, const unsigned char *key, unsigned char **value)
{
    int ret = 0;
    int len = 0;
    ...
    if (reply == NULL || reply->str == NULL) {
        fprintf(stderr, "[-][GMS-REDIS]Get database %s error!\n", key);
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "Get database %s error! %s\n", key, conn->errstr);
        if (reply->str == NULL) {
            ret = -2;
        }
        else {
	...
	printf("[+][GMS_REDIS]Get database %s SUCCESS!\n", key);
	LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[+][GMS_REDIS]Get database %d SUCCESS!\n", key);
 ...
 */
/* -------------------------------------------*/
int Rop_SetValue(redisContext *conn, const unsigned char *key, unsigned char *value)
{
    int ret = 0;
    redisReply *reply = NULL;
    ...
    if (reply == NULL || reply->str == NULL) {
        fprintf(stderr, "[-][GMS-REDIS]SET database %s %s error!\n", key, value);
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "Get database %s %s error! %s\n", key, value, conn->errstr);
        ret = -1;
        goto END;
    }

	printf("[+][GMS_REDIS]Set database %s %s SUCCESS!\n", key, value);
	LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[+][GMS_REDIS]Get database %s %s SUCCESS!\n", key, value);
